...
from reddelectrum.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from reddelectrum import bitcoin
from reddelectrum.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            ...
            self.app.show_info(_("Not a Reddcoin URI"))
            return
    ...
    def do_save(self):
        ...
        from reddelectrum.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Reddcoin address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Reddcoin Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from reddelectrum.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Reddcoin Request"))
